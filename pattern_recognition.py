...
#################################################
def activity_string_generator(data_dict):
	...
	for entry in sorted_dicts:
		...
		# add 30 minute blocks of o -- short idle time
		if delta_t >= 900. and delta_t <10800.:
			while delta_t >900:
				delta_t += -1800
				activity_string += "o,"
		# if time taken is greater than 3 hours, add one x -- long idle time
		elif delta_t >=10800.:
			activity_string += "x,"
...
names = [x for _,x in sorted(zip(values, names), reverse=True)][:200]
...
for name in names:
	...
	new_data = activity_string_generator(cut_data)##, color = color)
	all_data.append(new_data)


"""
longest_string = max(all_data, key=len)
longest_index = all_data.index(longest_strong)

for i in range(len(all_data)):
	#compare longest vs all other entries
	if i != longest_index:
		matches = SequenceMatcher(None, all_data[i], longest_string).get_matching_blocks()
