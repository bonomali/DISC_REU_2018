...
from io import open
from os import path
#from time import timeCommand "python setup.py egg_info" failed with error code 1 in /tmp/pip-install-w82xipy9/multiprocessing/
...
class Graph(defaultdict):
  ...
  def number_of_edges(self):
    ...
    change = list(self.keys())
    print(change)
    print(change[0])
    print(self.degree(change[0]))
    #print([self.degree(x) for x in change])
    #return sum([self.degree(x) for x in change])/2
...
def load_edgelist(file_, undirected = False):
	G = Graph()
	#f =open(file_, 'rb')
	G =nx.read_edgelist(file_,create_using=nx.DiGraph())
"""
